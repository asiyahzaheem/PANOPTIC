"""
Runs a ResNet on all CT scans and saves the embeddings for GNN training
"""
...
# Builts preprocessing pipeline for CT slices
...
def main():
    ...
    qc_log = artifacts / cfg["data"]["imaging_qc_log_csv"]
    bad_set = set()
    if qc_log.exists() and qc_log.stat().st_size > 0: # skip scans that faiiled QC
        try:
    ...
    for xk, pid, source in tqdm(loader, desc="Embeddings"):
        ...
        xflat = xk.view(B * K, C, H, W) # flatten batch and slcies for forward pass
        zflat = embedder(xflat) # [B*K,512]
        zscan = zflat.view(B, K, -1).mean(1) # average embeddings over slices
        zscan_cpu = zscan.cpu()
    ...
    print(f"Wrote: {out_pt}  (dict patient_id -> tensor[512])!")
