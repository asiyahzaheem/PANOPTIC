"""
Converts CPTAC DICOM folders into one NIfTI per patient
"""
...
def main():
    ...
    for pdir in tqdm(patients, desc="DICOM→NIfTI"):
        ...
        if out.exists() and out.stat().st_size > 0: # NIfTI already exists
            continue
        ...
        if not ok:
            failed += 1
            print(f"WARNING: No valid series for {pid} (skipping)") # Log failed patients

    print(f"Conversion done. failed={failed}. output={nifti_root}!")
